...
def main(unused_argv):
  ...
  for video_file, labels in csv.reader(open(FLAGS.input_videos_csv)):
    ...
    for rgb in frame_iterator(
      ...
      rgb_features.append(_float_list_feature(quantize(features)))
    ...
    rgb_features=np.mean(rgb_features,axis=0)
    feature={
        FLAGS.image_features_key:tf.train.Feature(float_list=tf.train.BytesList(value=rgb_features))
    }
    if FLAGS.insert_zero_audio_features:
        feature['audio']=tf.train.Feature(_bytes_feature(_make_bytes([0]*128)))

    feature[FLAGS.labels_feature_key]=tf.train.Feature(_int64_feature(sorted(map(int,labels))))
    feature[FLAGS.video_file_key_feature_key]=tf.train>Feature(_bytes_feature(_make_bytes(map(ord,video_file))))
